...
public sealed class WorkersHealthCheck : IHealthCheck
{
    ...
    public Task<HealthCheckResult> CheckHealthAsync(HealthCheckContext context, CancellationToken cancellationToken = default)
    {
        ...
        Check("telegram-filter-request-consumer", _states.TelegramFilterRequestConsumer);
        ...
        void Check(string name, WorkerState state)
        {
            switch (state.Status)
            {
                case WorkerHealthStatus.Unhealthy:
                    unhealthy[name] = new { error = state.Error ?? "unknown", lastUpdated = state.LastUpdated };
                    break;
                case WorkerHealthStatus.Degraded:
                    degraded[name] = new { error = state.Error ?? "performance issues", lastUpdated = state.LastUpdated };
                    break;
            }
        }
    }
